...
from VoxelProt.surface2Octree.shufflekeytable import *
class Octree:
  ...
  def initialize(self):
    ...
    self.features = [None] * (self.depth + 1)
    self.normals = [None] * (self.depth + 1)
  ...
  def build_octree(self, point_cloud):
    ...
    points = ((point_cloud.points + 1.0) * (2 ** (self.depth - 1)))
    ...
    normals = self.scatter_addition(point_cloud.normals, idx, dim=0).to(self.device)
    self.normals[self.depth] = F.normalize(normals).to(self.device)

    features = self.scatter_addition(point_cloud.features, idx, dim=0).to(self.device)
    self.features[self.depth] = features / counts.unsqueeze(1).to(self.device)
  ...
  def get_input_signal(self):
    ...
    # normals
    features = []
    features.append(self.normals[self.depth])
