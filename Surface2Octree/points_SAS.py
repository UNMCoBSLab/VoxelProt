...
class Points:
  ...
  def generateRoot(self,origin,boxlength):
    ...
    ori=origin.clone().detach() .cpu().numpy()
    radius=0.5*boxlength*math.sqrt(3)      
    sps=self.points.clone().detach().cpu().numpy()
    distances2= np.sum((sps-ori)**2, axis=1)
    rng=np.where(distances2<radius**2)[0]
    rv=[]
 
    for ind in range(len(rng)):
      x,y,z=self.points[rng[ind]].detach().numpy()
